...
- **Workflows:** Use `GET /repos/{owner}/{repo}/actions/workflows` to list workflows, then `GET /repos/{owner}/{repo}/actions/workflows/{id}/runs?branch={branch}&per_page=1` to get only the latest run per workflow+branch. Before fetching runs, apply the repo's `workflow_include`/`workflow_exclude` glob patterns (via `fnmatch`) to skip irrelevant workflows. If the branch-filtered request returns no runs **and the branch is the repo's default branch**, fall back to `GET /repos/{owner}/{repo}/actions/workflows/{id}/runs?per_page=1` (no `branch` param) to get the latest run overall, and report its status under the default branch. This covers workflows triggered by non-branch events (e.g. `release`, tag pushes) whose `head_branch` is a tag rather than the default branch. The fallback does **not** apply to non-default tracked branches: periodic or release-only workflows never actually ran there and must not be shown as if they did.
- Use `per_page=100` on all list endpoints to minimize pagination.
