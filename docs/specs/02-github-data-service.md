...
- **Workflows:** Use `GET /repos/{owner}/{repo}/actions/workflows` to list workflows, then `GET /repos/{owner}/{repo}/actions/workflows/{id}/runs?branch={branch}&per_page=1` to get only the latest run per workflow+branch. Before fetching runs, apply the repo's `workflow_include`/`workflow_exclude` glob patterns (via `fnmatch`) to skip irrelevant workflows. If the branch-filtered request returns no runs (common for workflows triggered by non-branch events like `release` or tag pushes, whose `head_branch` is the tag rather than a tracked branch), fall back to `GET /repos/{owner}/{repo}/actions/workflows/{id}/runs?per_page=1` (no `branch` param) to get the latest run overall, and report its status under the originally configured branch.
- Use `per_page=100` on all list endpoints to minimize pagination.
